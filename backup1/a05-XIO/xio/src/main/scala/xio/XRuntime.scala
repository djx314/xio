...
import xio.nat.has.{Nat, NatToTag, NatZero}
import zio._
...
trait XRuntime[R <: Nat] {
  ...
  final def unsafeRunTask[R1 <: Nat, A](task: => XIO[R1, NatEitherPositive[NatEitherZero, Throwable], A])(implicit n: NatToTag[R1, R]): A =
    self.zioRuntime.map(n.tag).unsafeRunTask(task.zio.mapError(_.sureRight))
...
object XRuntime {
  ...
  object XEnv {
    ...
    val xlive: XEnv =
      XHas5(clock.Clock.Service.live, console.Console.Service.live, system.System.Service.live, random.Random.Service.live, blocking.Blocking.Service.live)
    ...
    val live: XLayer[XHas0, XError0, XEnv] = {
      val live1: XLayer[XHas0, XError0, XHas1[clock.Clock.Service]] =
        new XLayer[NatZero, XError0, XHas1[clock.Clock.Service]] {
          override val zlayer: ZLayer[XHas0, XError0, XHas1[clock.Clock.Service]] = clock.Clock.live.map(r => XHas1(r.get))
        }
      val live2: XLayer[XHas0, XError0, XHas1[console.Console.Service]] =
        new XLayer[XHas0, XError0, XHas1[console.Console.Service]] {
          override val zlayer: ZLayer[XHas0, XError0, XHas1[console.Console.Service]] =
            console.Console.live.map(r => XHas1(r.get))
        }
      val live3: XLayer[XHas0, XError0, XHas1[system.System.Service]] =
        new XLayer[XHas0, XError0, XHas1[system.System.Service]] {
          override val zlayer: ZLayer[XHas0, NatEitherZero, XHas1[system.System.Service]] = system.System.live.map(r => XHas1(r.get))
        }
      val live4: XLayer[XHas0, XError0, XHas1[random.Random.Service]] =
        new XLayer[XHas0, XError0, XHas1[random.Random.Service]] {
          override val zlayer: ZLayer[XHas0, XError0, XHas1[random.Random.Service]] = random.Random.live.map(r => XHas1(r.get))
        }
      val live5: XLayer[XHas0, XError0, XHas1[blocking.Blocking.Service]] =
        new XLayer[XHas0, XError0, XHas1[blocking.Blocking.Service]] {
          override val zlayer: ZLayer[XHas0, XError0, XHas1[blocking.Blocking.Service]] =
            blocking.Blocking.live.map(r => XHas1(r.get))
        }
