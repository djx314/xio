...
object NatToTag {
  implicit def zeroNat: NatToTag[TagNatZero] =
    new NatToTag[TagNatZero] {
      override def tag: TagNatZero = TagNatZero.value
    }

  implicit def positiveNat[Head, TagTail <: TagNat](implicit
    to: NatToTag[TagTail]
  ): NatToTag[TagNatPositive[TagTail, Head]] =
