...
object XErrorReversePlus {
  implicit def firstNat[N2 <: XError, F](implicit n: ZeroSetter[N2]): XErrorReversePlus[N2, XErrorZero] = new XErrorReversePlusFirst(n = n)
...
class XErrorReversePlusFirst[N2 <: XError](n: ZeroSetter[N2]) extends XErrorReversePlus[N2, XErrorZero] {
  override type PluI = N2
  override def plus(t1: N2): Either[XErrorZero, N2] = Right(t1)
  override def takeHead(t1: XErrorZero): N2         = n.tag(t1)
