...
import xio.nat.has.{Nat, NatPositive}

trait 自然数标记 {
  type 自然数消去[T <: Nat, I] <: 自然数对
  type 加[I <: 自然数标记] <: 自然数标记
  type 乘以[I <: 自然数标记] <: 自然数标记
  def 自然数消去[T <: Nat, I](item: T, i: I): 自然数消去[T, I]
...
class 自然数标记零 extends 自然数标记 {
  self =>
  override type 自然数消去[T <: Nat, I] = 正整数自然数对[T, I]
  override type 加[I <: 自然数标记]      = I
  override type 乘以[I <: 自然数标记]     = 自然数标记零
  override def 自然数消去[T <: Nat, I](item: T, i: I): 正整数自然数对[T, I] = new 正整数自然数对(self, new NatPositive(tail = item, head = i))
...
class 正整数标记[Tail <: 自然数标记](val tail: Tail) extends 自然数标记 {
  self =>
  override type 自然数消去[T <: Nat, I] = T#消去标记[Tail]
  override type 加[I <: 自然数标记]      = 正整数标记[Tail#加[I]]
  override type 乘以[I <: 自然数标记]     = I#加[Tail#乘以[I]]
  override def 自然数消去[T <: Nat, I](item: T, i: I): T#消去标记[Tail] = item.消去标记(tail)
